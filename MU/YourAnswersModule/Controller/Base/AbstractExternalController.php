...
abstract class AbstractExternalController extends AbstractController
{
    ...
    public function displayAction(Request $request, $objectType, $id, $source, $displayMode)
    {
        ...
        $entityFactory = $this->get('mu_youranswers_module.entity_factory');
        ...
        if (!$this->get('mu_youranswers_module.permission_helper')->mayRead($entity)) {
            return '';
        }
        ...
        $contextArgs = ['controller' => 'external', 'action' => 'display'];
        $templateParameters = $this->get('mu_youranswers_module.controller_helper')->addTemplateParameters($objectType, $templateParameters, 'controllerAction', $contextArgs);
        
        return $this->render('@MUYourAnswersModule/External/' . ucfirst($objectType) . '/' . $template, $templateParameters);
    }
    ...
    public function finderAction(Request $request, $objectType, $editor, $sort, $sortdir, $pos = 1, $num = 0)
    {
        ...
        $cssAssetBag->add($assetHelper->resolve('@MUYourAnswersModule:css/style.css'));
        $cssAssetBag->add([$assetHelper->resolve('@MUYourAnswersModule:css/custom.css') => 120]);
        
        $listEntriesHelper = $this->get('mu_youranswers_module.listentries_helper');
        ...
        if (!in_array($objectType, $activatedObjectTypes)) {
            ...
            $redirectUrl = $this->get('router')->generate('muyouranswersmodule_external_finder', ['objectType' => array_shift($activatedObjectTypes), 'editor' => $editor]);
        
            return new RedirectResponse($redirectUrl);
        }
        
        if (!$this->get('mu_youranswers_module.permission_helper')->hasComponentPermission($objectType, ACCESS_COMMENT)) {
            throw new AccessDeniedException();
        }
        ...
        $form = $this->createForm('MU\YourAnswersModule\Form\Type\Finder\\' . ucfirst($objectType) . 'FinderType', $templateParameters, $formOptions);
        ...
        if ('' != $searchTerm) {
            $qb = $this->get('mu_youranswers_module.collection_filter_helper')->addSearchFilter($objectType, $qb, $searchTerm);
        }
        ...
        $output = $this->renderView('@MUYourAnswersModule/External/' . ucfirst($objectType) . '/find.html.twig', $templateParameters);

...
namespace MU\YourAnswersModule\Form\Handler\Common\Base;
...
use MU\YourAnswersModule\Helper\HookHelper;
use MU\YourAnswersModule\Helper\ModelHelper;
...
abstract class AbstractEditHandler
{
    ...
    public function processForm(array $templateParameters = [])
    {
        ...
        // default to referer
        $refererSessionVar = 'muyouranswersmodule' . $this->objectTypeCapital . 'Referer';
        ...
        if ($this->templateParameters['mode'] == 'edit') {
            ...
            if (null !== $entity) {
                if (true === $this->hasPageLockSupport && $this->kernel->isBundle('ZikulaPageLockModule') && null !== $this->lockingApi) {
                    ...
                    $lockName = 'MUYourAnswersModule' . $this->objectTypeCapital . $entity->getKey();
            ...
            $permissionLevel = in_array($this->objectType, ['answer', 'question']) ? ACCESS_COMMENT : ACCESS_EDIT;
        ...
        if (false === $actions || !is_array($actions)) {
            ...
            $logArgs = ['app' => 'MUYourAnswersModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType, 'id' => $entity->getKey()];
    ...
    public function handleCommand(array $args = [])
    {
        ...
        if ($entity->supportsHookSubscribers()) {
            ...
            if ($hasDisplayAction && $action != 'delete') {
                ...
                $routeUrl = new RouteUrl('muyouranswersmodule_' . $this->objectTypeLower . '_display', $urlArgs);
            }
    ...
    protected function addDefaultMessage(array $args = [], $success = false)
    {
        ...
        $logArgs = ['app' => 'MUYourAnswersModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType, 'id' => $this->entityRef->getKey()];
        if (true === $success) {
    ...
    public function fetchInputData()
    {
        ...
        if (isset($this->form['additionalNotificationRemarks']) && $this->form['additionalNotificationRemarks']->getData() != '') {
            $this->requestStack->getCurrentRequest()->getSession()->set('MUYourAnswersModuleAdditionalNotificationRemarks', $this->form['additionalNotificationRemarks']->getData());
        }
    ...
    protected function prepareWorkflowAdditions($enterprise = false)
    {
        ...
        $groupApplicationArgs = [
            'user' => $currentUserId,
            'group' => $this->variableApi->get('MUYourAnswersModule', 'moderationGroupFor' . $this->objectTypeCapital, GroupsConstant::GROUP_ID_ADMIN)
        ];
        ...
        if (true === $enterprise) {
            $groupApplicationArgs = [
                'user' => $currentUserId,
                'group' => $this->variableApi->get('MUYourAnswersModule', 'superModerationGroupFor' . $this->objectTypeCapital, GroupsConstant::GROUP_ID_ADMIN)
            ];

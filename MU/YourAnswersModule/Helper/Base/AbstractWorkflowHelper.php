...
abstract class AbstractWorkflowHelper
{
    ...
    public function collectAmountOfModerationItems()
    {
        ...
        if ($this->permissionHelper->hasComponentPermission($objectType, ACCESS_ADD)) {
            ...
            if ($amount > 0) {
                $amounts[] = [
                    'aggregateType' => 'answersApproval',
                    'description' => $this->translator->__('Answers pending approval'),
                    'amount' => $amount,
                    'objectType' => $objectType,
                    'state' => $state,
                    'message' => $this->translator->_fn('One answer is waiting for approval.', '%amount% answers are waiting for approval.', $amount, ['%amount%' => $amount])
                ];
        
                $this->logger->info('{app}: There are {amount} {entities} waiting for approval.', ['app' => 'MUYourAnswersModule', 'amount' => $amount, 'entities' => 'answers']);
            }
        }
        $objectType = 'question';
        if ($this->permissionHelper->hasComponentPermission($objectType, ACCESS_ADD)) {
            ...
            if ($amount > 0) {
                $amounts[] = [
                    'aggregateType' => 'questionsApproval',
                    'description' => $this->translator->__('Questions pending approval'),
                    'amount' => $amount,
                    'objectType' => $objectType,
                    'state' => $state,
                    'message' => $this->translator->_fn('One question is waiting for approval.', '%amount% questions are waiting for approval.', $amount, ['%amount%' => $amount])
                ];
        
                $this->logger->info('{app}: There are {amount} {entities} waiting for approval.', ['app' => 'MUYourAnswersModule', 'amount' => $amount, 'entities' => 'questions']);
            }
        }

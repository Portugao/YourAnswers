...
abstract class AbstractArchiveHelper
{
    ...
    public function archiveObsoleteObjects($probabilityPercent = 75)
    {
        ...
        // perform update for questions becoming archived
        $logArgs = ['app' => 'MUYourAnswersModule', 'entity' => 'question'];
    ...
    /**
     * Moves questions into the archive which reached their end date.
     *
     * @throws RuntimeException Thrown if workflow action execution fails
     */
    protected function archiveQuestions()
    {
        $today = date('Y-m-d H:i:s');
    
        $affectedEntities = $this->getObjectsToBeArchived('question', 'endDate', $today);
    ...
    protected function archiveSingleObject($entity)
    {
        ...
        if ($entity->supportsHookSubscribers()) {
            ...
            $hasDisplayPage = in_array($objectType, ['answer', 'question']);
            if ($hasDisplayPage) {
                $urlArgs = $entity->createUrlArgs();
                if (null !== $request) {
                    $urlArgs['_locale'] = $request->getLocale();
                }
                $url = new RouteUrl('muyouranswersmodule_' . strtolower($objectType) . '_display', $urlArgs);
        	}

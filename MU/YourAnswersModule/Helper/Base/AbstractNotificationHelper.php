...
abstract class AbstractNotificationHelper
{
    ...
    protected function collectRecipients($debug = false)
    {
        ...
        if ($this->recipientType == 'moderator' || $this->recipientType == 'superModerator') {
            ...
            $moderatorGroupId = $this->variableApi->get('MUYourAnswersModule', 'moderationGroupFor' . $modVarSuffix, GroupsConstant::GROUP_ID_ADMIN);
            if ($this->recipientType == 'superModerator') {
                $moderatorGroupId = $this->variableApi->get('MUYourAnswersModule', 'superModerationGroupFor' . $modVarSuffix, GroupsConstant::GROUP_ID_ADMIN);
            }
    ...
    protected function sendMails()
    {
        ...
        foreach ($this->recipients as $recipient) {
            ...
            $body = $this->templating->render('@MUYourAnswersModule/' . $template, [
                'recipient' => $recipient,
                'mailData' => $mailData
            ]);
            $altBody = '';
    ...
    protected function prepareEmailData()
    {
        ...
        $hasDisplayAction = in_array($objectType, ['answer', 'question']);
        $hasEditAction = in_array($objectType, ['answer', 'question']);
        $routeArea = in_array($this->recipientType, ['moderator', 'superModerator']) ? 'admin' : '';
        $routePrefix = 'muyouranswersmodule_' . strtolower($objectType) . '_' . $routeArea;

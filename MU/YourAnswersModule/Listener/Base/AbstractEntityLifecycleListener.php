...
use MU\YourAnswersModule\Event\FilterAnswerEvent;
use MU\YourAnswersModule\Event\FilterQuestionEvent;
...
abstract class AbstractEntityLifecycleListener implements EventSubscriber, ContainerAwareInterface
{
    ...
    public function preRemove(LifecycleEventArgs $args)
    {
        ...
        $this->eventDispatcher->dispatch(constant('\\MU\\YourAnswersModule\\YourAnswersEvents::' . strtoupper($entity->get_objectType()) . '_PRE_REMOVE'), $event);
    ...
    public function postRemove(LifecycleEventArgs $args)
    {
        ...
        $logArgs = ['app' => 'MUYourAnswersModule', 'user' => $currentUserApi->get('uname'), 'entity' => $objectType, 'id' => $entity->getKey()];
        $this->logger->debug('{app}: User {user} removed the {entity} with id {id}.', $logArgs);
        
        // create the filter event and dispatch it
        $event = $this->createFilterEvent($entity);
        $this->eventDispatcher->dispatch(constant('\\MU\\YourAnswersModule\\YourAnswersEvents::' . strtoupper($objectType) . '_POST_REMOVE'), $event);
    }
    ...
    public function prePersist(LifecycleEventArgs $args)
    {
        ...
        // create the filter event and dispatch it
        $event = $this->createFilterEvent($entity);
        $this->eventDispatcher->dispatch(constant('\\MU\\YourAnswersModule\\YourAnswersEvents::' . strtoupper($entity->get_objectType()) . '_PRE_PERSIST'), $event);
        if ($event->isPropagationStopped()) {
            return false;
        }
    }
    ...
    public function postPersist(LifecycleEventArgs $args)
    {
        ...
        $currentUserApi = $this->container->get('zikula_users_module.current_user');
        $logArgs = ['app' => 'MUYourAnswersModule', 'user' => $currentUserApi->get('uname'), 'entity' => $entity->get_objectType(), 'id' => $entity->getKey()];
        $this->logger->debug('{app}: User {user} created the {entity} with id {id}.', $logArgs);
        
        // create the filter event and dispatch it
        $event = $this->createFilterEvent($entity);
        $this->eventDispatcher->dispatch(constant('\\MU\\YourAnswersModule\\YourAnswersEvents::' . strtoupper($entity->get_objectType()) . '_POST_PERSIST'), $event);
    }
    ...
    public function preUpdate(PreUpdateEventArgs $args)
    {
        ...
        // create the filter event and dispatch it
        $event = $this->createFilterEvent($entity);
        $this->eventDispatcher->dispatch(constant('\\MU\\YourAnswersModule\\YourAnswersEvents::' . strtoupper($entity->get_objectType()) . '_PRE_UPDATE'), $event);
        if ($event->isPropagationStopped()) {
            return false;
        }
    }
    ...
    public function postUpdate(LifecycleEventArgs $args)
    {
        ...
        // create the filter event and dispatch it
        $event = $this->createFilterEvent($entity);
        $this->eventDispatcher->dispatch(constant('\\MU\\YourAnswersModule\\YourAnswersEvents::' . strtoupper($entity->get_objectType()) . '_POST_UPDATE'), $event);
    }
    ...
    public function postLoad(LifecycleEventArgs $args)
    {
        ...
        // create the filter event and dispatch it
        $event = $this->createFilterEvent($entity);
        $this->eventDispatcher->dispatch(constant('\\MU\\YourAnswersModule\\YourAnswersEvents::' . strtoupper($entity->get_objectType()) . '_POST_LOAD'), $event);
    }
    ...
    protected function isEntityManagedByThisBundle($entity)
    {
        ...
        return ($entityClassParts[0] == 'MU' && $entityClassParts[1] == 'YourAnswersModule');
    }
    ...
    {
        $filterEventClass = '\\MU\\YourAnswersModule\\Event\\Filter' . ucfirst($entity->get_objectType()) . 'Event';
